...
| settings.ai.providers_help | ONNX Runtime providers for InsightFace. auto (default): CUDA if installed, else CPU. CoreML is skipped (incompatible with InsightFace SCRFD). Example: auto or CPUExecutionProvider | ONNX-Runtime-Provider für InsightFace. „auto“ (Standard): CUDA (falls installiert), sonst CPU. CoreML wird übersprungen (nicht kompatibel mit InsightFace SCRFD). Beispiel: auto oder CPUExecutionProvider |
